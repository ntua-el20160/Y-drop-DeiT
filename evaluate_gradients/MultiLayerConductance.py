...
class MultiLayerConductance(LayerAttribution, GradientAttribution):
    r"""
    ...
    def attribute(
        ...
        return_input_attributions: bool = False,
        attribute_to_layer_input: bool = False,
        grad_kwargs: Optional[Dict[str, Any]] = None,
    ) -> Union[
        Tensor,
    ...
    # ]:
        ...
        input_attr = None
        if return_input_attributions:
            from captum.attr import IntegratedGradients

            ig = IntegratedGradients(self.forward_func)
            # we pass exactly the same args, but IG will hook the inputs
            input_attr, ig_delta = ig.attribute(
                inputs,
                baselines=baselines,
                target=target,
                additional_forward_args=additional_forward_args,
        ...
        if return_convergence_delta:
            ...
            delta =[]
            for i, attr in enumerate(attributions[0]):
                delta.append(self.compute_convergence_delta(
                    attr,
                    start_point,
                    end_point,
                    target=target,
            ...
            if return_input_attributions:
                return _format_output(is_layer_tuple, attributions),input_attr, delta
            else:
                return _format_output(is_layer_tuple, attributions), delta
        if return_input_attributions: 
            return _format_output(is_layer_tuple, attributions), input_attr
    ...
    def _attribute(
        ...
        # by the step size.
        if isinstance(layer_evals, tuple):
        # Single layer: each element in layer_evals is a tensor.
            grad_diffs = tuple(
                layer_eval[num_examples:] - layer_eval[:-num_examples]
                for layer_eval in layer_evals
            )
        else:
            # Multiple layers: layer_evals is a list of tuples.
            grad_diffs = []
            for layer_eval_tuple in layer_evals:
                # For each layer, compute the differences for each tensor in its tuple.
                layer_grad_diff = tuple(
        ...
        if isinstance(layer_gradients, tuple):
            # Single layer: use existing logic.
            attributions = tuple(
                _reshape_and_sum(
            ...
            # Multiple layers: each of layer_gradients, layer_evals, grad_diffs is a list of tuples.
            attributions = []
            for grad_tuple, eval_tuple, grad_diff_tuple in zip(layer_gradients, layer_evals, grad_diffs):
                # Compute attributions for each tensor in the tuple for this layer.
                layer_attributions = tuple(
                    _reshape_and_sum(
                        grad_diff * grad_tensor[:-num_examples],
                        n_steps,
                        num_examples,
                        tuple(eval_tensor.shape[1:]),
                    )
                    for grad_tensor, eval_tensor, grad_diff in zip(grad_tuple, eval_tuple, grad_diff_tuple)
                )
                attributions.append(layer_attributions)

        if isinstance(self.layer, Module):
            return _format_output(False, attributions)
        else:
            processed_attributions = [
            attr[0] if isinstance(attr, tuple) and len(attr) == 1 else attr
